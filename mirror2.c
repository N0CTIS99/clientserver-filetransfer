...
const char *homedir = "/home/turjoa"; //home directory
int searchResult = 0;
...
int fileSizeSearch(const char *fpath, const struct stat *sb, int typeflag, struct FTW *ftwbuf){
    ...
    if (typeflag == FTW_F){
        ...
        // fprintf(stdout, "Current file size is %d bytes", fsize);
...
int findFilesByExtension(const char *fpath, const struct stat *sb, int typeflag, struct FTW *ftwbuf){
    ...
    if(typeflag==FTW_F){ //checking the traversed directory is a regular file or not.
        if(getFilesWithSameExt(fpath) == 0){ //if the getFilesWithSameExt returns 0 then it will proceed with the same extension
...
int crequest(int cfd){
    ...
    char* serverMsg = "Connected to mirror 2...";
...
int main(int argc, char *argv[]){
    ...
    sAdrStr.sin_port = htons(7997);     //converting the 16 bit host byte order of port number to network byte order
